...
def lplb_algorithm(phy2log: torch.Tensor, 
                logcnt: torch.Tensor, 
    ...
    for layer_id in range(phy2log.shape[0]):
        ...
        log2phy_prob = single_layer_lplb_algorithm(layer_phy2log, layer_logcnt, layer_log2phy, g, logical_count, device)
        log2phy_prob[layer_id] = log2phy_prob
    return log2phy_prob

def single_layer_lplb_algorithm(layer_phy2log: torch.Tensor, 
    ...
    x[x < 0.0] = 0.0
    phy_prob = torch.zeros(log_single_expert_array.shape[0]
