...
class TokenDispatchMetadata:
    ...
    def init(phy2log: torch.Tensor, 
        ...
        logcnt = torch.zeros((num_layers, num_logical), dtype=torch.int, device=phy2log.device)
        ...
        for i in range(num_layers):
            A_i, B1_i, B2_i, c_i, log_single_expert_array_i, phy_single_expert_array_i, log_replicated_expert_array_i, phy_replicated_expert_array_i = TokenDispatchMetadata.init_single_layer(phy2log[i], logcnt[i], log2phy[i], g)
            A.append(A_i)
            B1.append(B1_i)
            B2.append(B2_i)
            c.append(c_i)
        ...
        return TokenDispatchMetadata(A, B1, B2, c, log_single_expert_array, phy_single_expert_array, log_replicated_expert_array, phy_replicated_expert_array)

    @staticmethod
    def init_single_layer(layer_phy2log: torch.Tensor, 
                          layer_logcnt: torch.Tensor, 
                          layer_log2phy: torch.Tensor,
                          g: int):
        assert layer_phy2log.device == layer_logcnt.device, f"layer_phy2log and layer_logcnt must be on the same device, {layer_phy2log.device=} != {layer_logcnt.device=}"
        ...
        log_replicated_expert_array: torch.Tensor = torch.nonzero(layer_logcnt > 1).flatten()
        ...
        C = torch.zeros((log_replicated_expert_count, phy_replicated_expert_count), 
        ...
        zeros_top = torch.zeros((log_replicated_expert_count, g), dtype=torch.float32, device=device)
        zeros_top_col = torch.zeros((log_replicated_expert_count, 1), dtype=torch.float32, device=device)
        ...
        return A, B1, B2, c, log_single_expert_array, phy_single_expert_array, log_replicated_expert_array, phy_replicated_expert_array
...
def test():
    ...
    logcnt = torch.cat([torch.full((32,), 2), torch.ones(224, dtype=torch.long)]).repeat(58, 1).to(device)
    log2phy = torch.cat(
        [
            torch.stack([torch.arange(32), torch.arange(32) + 256], dim=1).to(device),
            torch.stack([torch.arange(32, 256), torch.full((224,), -1)], dim=1).to(device),
        ],
        dim=0,
    ).repeat(58, 1, 1).to(device)
    ...
    for attr in ['A', 'B1', 'B2', 'single_expert_array', 'log_replicated_expert_array', 'phy_replicated_expert_array']:
